...
def buy_handler(client: Client, message: types.Message):
    ...
    markup = types.InlineKeyboardMarkup(
        [
            [
                types.InlineKeyboardButton("Bot Payments(Disabled)", callback_data=f"bot-payments-{price}"),
                types.InlineKeyboardButton("TRON(TRX Disabled)", callback_data="tron-trx"),
            ],
        ]
    )
...
def tronpayment_btn_calback(client: Client, callback_query: types.CallbackQuery):
    callback_query.answer("Feature Disabled Sorry For Inconvinience...")
    chat_id = callback_query.message.chat.id
    client.send_chat_action(chat_id, enums.ChatAction.TYPING)

    addr = TronTrx().get_payment_address(chat_id)
    with BytesIO() as bio:
        client.send_message(chat_id, caption=f"Send any amount of TRX to `TKFjNFYmnMaRBpsLkNdjpDkKUgMnCq2ChP`")
